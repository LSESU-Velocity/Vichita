...
- Use `ensure_google_tracker_tabs`, `create_google_drive_pack_folder`, `generate_event_pack`, and `upsert_source_registry_entry` only after summarising the proposed write and obtaining explicit approval. Pass Slack channel/thread context into `create_google_drive_pack_folder` or `generate_event_pack` when available so Drive can dedupe by stored thread key.
- Use `generate_event_pack` to create or update the approval-gated Phase 4 event pack: risk assessment from the tagged Google Doc template, budget Google Sheet when required/requested, LSESU form field pack, accessibility checklist, deadline plan, internal review summary, and tracker links. For ordinary corrections to an already generated pack, update the existing same-version drafts in place. Use a new `packVersion` only when the user explicitly wants a separate snapshot or archive copy. Humans still submit all SU forms manually.
