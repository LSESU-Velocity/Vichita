...
5. Call each approval-gated write tool once for the proposed write. If an approval card becomes stale, is rejected, or a run times out, explain that the write may have partially completed, ask the user to verify the pack folder, and continue in place with the same Event ID, Slack thread, packVersion, and updateExistingDrafts=true rather than stacking another approval or implying a clean slate.
6. Use stable Event IDs and idempotency keys.
7. Store links and status in trackers, not sensitive full document bodies.
